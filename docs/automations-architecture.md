...
**Lo que el envío necesita y no configura la regla:** la plantilla APROBADA por Meta de la organización (ítem 160: la arma cada negocio desde Administración → Plantilla de WhatsApp, tabla `whatsapp_templates`; dos variables posicionales: `{{1}}` nombre del contacto, `{{2}}` `QrCode.destinationUrl`), `WHATSAPP_ACCESS_TOKEN`, y un agente de la sucursal del QR con número de WhatsApp conectado (`Agent.whatsappPhoneNumberId`) — el número del que sale el mensaje. Sin el token, el worker no reclama nada y lo dice en el log en cada pasada; sin plantilla aprobada, las filas de esa organización no se reclaman (quedan en `PENDING` sin gastar intentos); sin número o sin teléfono del contacto, la fila pasa a `FAILED` con el motivo. *(Hasta el ítem 160 la plantilla era una sola para toda la plataforma, en `WHATSAPP_REVIEW_FOLLOWUP_TEMPLATE_NAME` / `_LANGUAGE`.)*
