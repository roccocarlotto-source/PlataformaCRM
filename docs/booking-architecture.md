...
- `POST /api/webhooks/google-calendar` — receptor de notificaciones push de Google (cambios externos). *(Construido el 31/08/2026.)*
  ...
  >
  > **Los códigos de respuesta SON el mecanismo de reintento** — no hay cola propia, y es deliberado: Google ya reintenta con backoff ante un error del servidor, y construir un outbox para esto duplicaría un mecanismo que el proveedor ya da, para un volumen que no lo justifica. `200` procesado o nada que hacer; `403` token que no salió de acá; `5xx` algo se rompió de este lado y conviene que reintente. **Un canal desconocido devuelve 200**, no 5xx: si no, Google reintentaría para siempre una notificación que nunca vamos a poder procesar. **Precisión del 02/09/2026 (V-1 de la auditoría del 29/08):** esta nota decía "ante una respuesta no-2xx", y no es exacto — la guía de push de Calendar reintenta con backoff **solo** ante `500`, `502`, `503` o `504`; "every other return status code is considered to be a message failure". Un `4xx` no se reintenta: el `403` del token corta el reintento y no acepta el canal espurio. Y ese `403` es lógica viva, no un detalle: `googleCalendarWebhook.controller.ts` despacha por su valor (vía la constante compartida `STATUS_TOKEN_DE_CANAL_INVALIDO` de `webhookToken.ts`); cualquier otro error cae al `503` genérico, que sí se reintenta.
  >
