...
| M-29 | **El email de un contacto no se puede vaciar por `PATCH`.** `contact.service.ts:196` hace `normalizeEmail(input.email ?? undefined)`: con `input.email === null` —la forma de pedir "borrá este campo"— devuelve `undefined`, y Prisma ignora `undefined` en un update. La operación responde 200 y no cambia nada. Es la contraparte backend de la falla del frontend registrada en ALTO-10: allá el cliente nunca mandaba el `null`, acá el servidor lo descarta si llega. Arreglar solo una de las dos mitades no resuelve el caso. Detectado el 2026-08-24 mientras se analizaba M-13 | `contact.service.ts:196` |
