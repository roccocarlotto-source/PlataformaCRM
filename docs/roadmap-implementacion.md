...
- [x] Webhook de sincronización inversa + worker de renovación de canales push (expiran a los ~7 días) — **hecho (PR #44)**. El "~7 días" se verificó contra la referencia de `events.watch` y **esta vez el documento tenía razón**: el TTL por defecto es 604800 s, 7 días exactos. Worker cada hora con 24 h de margen (absorbe un fin de semana de servidor caído). Estado del canal en `GoogleCalendarConnection` (`channelId`/`channelResourceId`/`channelExpiration`/`syncToken`), no en tabla nueva.
  - **La decisión de producto del paso**: un evento **cancelado** en Google cancela el `Booking` y libera el cupo (es seguro de automatizar: liberar un cupo no puede producir un estado inválido); un evento **movido** solo se registra en el log y **no** se aplica, porque eso es reprogramar y reprogramar exige revalidar horario + capacidad + Google. La reserva queda desactualizada hasta que alguien la resuelva a mano — aceptado, es el caso menos común.
  - **Lo que se verificó contra Google y corrigió una suposición**: `expiration` vuelve en **milisegundos** (leerlo como segundos daría 1970 y el worker recrearía el canal en cada pasada, para siempre, sin fallar visiblemente); y `nextSyncToken` viene **solo en la última página**, así que sin paginar con `nextPageToken` se guardaría un token que no reflejó todos los cambios — un hueco invisible.
  - [ ] **⚠️ Configuración externa pendiente de Rocco: verificar el dominio en Google Search Console** y registrarlo en la API Console (*Domain verification → Add domain*) antes de que la sincronización inversa funcione en producción. Es una medida anti-abuso de Google: sin eso rechaza la URL del canal. Análogo al proyecto de Google Cloud del paso 2. **No bloquea el merge ni el CI**, y el resto del módulo de agenda funciona igual.
