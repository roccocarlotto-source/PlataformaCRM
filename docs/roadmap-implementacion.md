...
- [x] Conexión OAuth con Google Calendar por sucursal + `freebusy.query` — **hecho (PR #42)**: modelo `GoogleCalendarConnection` + migración escrita a mano, flujo OAuth completo (iniciar / callback / desconectar / leer estado), y el wrapper aislado de `freebusy.query` con su test unitario mockeado. Tres cosas que aparecieron al implementar y no estaban en el diseño: (1) **la premisa "cifrar igual que `ApiKey`" era falsa** — `ApiKey` está *hasheada*, no cifrada, y no había ningún módulo de cifrado en el repo; se paró, se consultó por ser una decisión de seguridad, y salió `src/utils/encryption.ts` (AES-256-GCM, genérico, no atado a Google); (2) **`calendar.events` no habilita `freebusy.query`** — verificado contra la doc de Google, hacía falta `calendar.events.freebusy`, y pedir solo lo que decía el documento habría dado un 403 en la primera consulta de disponibilidad; (3) el `state` firmado es lo único que sostiene la frontera de tenant en el callback, que no puede estar autenticado. **`GET /api/availability` NO se construyó**, a propósito: ver el bullet de acá abajo.
  - [ ] **Pendiente que este paso destrabó y no podía resolver: el "rango de trabajo" por `Resource`.** `GET /api/availability` necesita saber qué días y horarios atiende cada recurso, y eso **no existe en el schema ni está diseñado en ningún documento** — es una frase suelta en `booking-architecture.md` §4/§5. Es una decisión de producto (¿horarios por recurso o por sucursal? ¿excepciones y feriados? ¿bloqueos puntuales?) y hay que tomarla antes de escribir el endpoint. Va junto con el paso de acá abajo, que aporta la otra mitad (`Booking`, para la capacidad ya ocupada).
- [ ] `POST /api/bookings` con creación de evento en Google + control de capacidad propio. **Tres cosas que quedaron preparadas y hay que terminar acá** (PR #41; están también en el comentario de `deleteServiceType`, no solo en este bullet): (1) `contarReservasActivas()` en `serviceType.service.ts` devuelve `0` fijo porque `Booking` no existe — reemplazarlo por el count real, decidiendo explícitamente si `COMPLETED`/`NO_SHOW` cuentan (probablemente no: son historia, no reservas vivas); (2) darle a `deleteServiceType` el lock de fila que hoy no tiene porque no hay nada contra qué serializar — sin (1) y (2) **juntos** el bloqueo es evitable por concurrencia, que es la lección de ALTO-8; (3) agregar `@@unique([organizationId, id])` a `ServiceType`, que se dejó afuera porque todavía nada lo referencia.
