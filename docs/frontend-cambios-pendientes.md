...
## 46. Rediseño de los `<select>`, Fase 2: el resto de los selects nativos + animación de apertura

**Estado:** hecho

**Contexto:** la Fase 1 (§44) dejó el combobox del design system (`Select`, `MultiSelect`) montado en dos pilotos, `UserSelect` y `BranchSelect`. Todo el resto de la app seguía con `<select>` nativos, que no se pueden restylear ni animar —el navegador dibuja el popup aparte, fuera del alcance de CSS— y que además se ven mal superpuestos con el resto de la UI. Rocco lo vio en el "Ordenar por" del listado de Empresas y confirmó que quería resolver todos los que quedaran, no solo ese. Esta ronda migra el resto y le agrega a los dos paneles una animación de apertura.

**Comportamiento deseado (confirmado con Rocco):**

- Ningún `<select>` nativo en la app, salvo los cuatro buscadores server-side que quedan fuera a propósito (ver alcance).
- Al abrirse, el panel entra con una animación sutil y rápida: opacidad 0→1 y un desplazamiento vertical chico, ~120-160 ms, easing tipo `ease-out`. Respetando `prefers-reduced-motion: reduce`.
- Se mantienen las reglas del §44: sin íconos decorativos (el chevron del control cerrado y el check de la fila elegida no cuentan), y subtítulo (segunda línea) solo en selectores que eligen una persona.

**Alcance real:** **60 elementos `<select>` en 27 archivos** — el mismo número que había anotado el §44, recontado antes de empezar. Por elemento, no por campo visible: `VehicleFormPage` tiene un solo `<select>` dentro de su `EnumField` local, que sirve a **10** campos de la ficha, y `OrganizationSettingsPage` uno dentro de `CurrencySelect`, que sirve a 2.

| Archivo | `<select>` | Archivo | `<select>` |
|---|---|---|---|
| `user/UserListPage` | 5 | `pipeline/PipelineListPage` | 2 |
| `apiKey/ApiKeyListPage` | 5 | `qr/QrListPage` | 2 |
| `source/SourceListPage` | 4 | `quote/QuoteFormPanel` | 2 |
| `opportunity/OpportunityFormPage` | 4 | `stage/StageSelect` | 2 |
| `activity/ActivityListPage` | 4 | `activity/ActivityFormPage` | 1 |
| `contact/ContactListPage` | 3 | `activity/MyTasksPage` | 1 |
| `ingestionEvent/IngestionEventListPage` | 3 | `branch/BranchFormPage` | 1 |
| `invitation/InvitationListPage` | 3 | `contact/ContactFormPage` | 1 |
| `opportunity/OpportunityListPage` | 3 | `invitation/InvitationFormPage` | 1 |
| `vehicle/VehicleListPage` | 3 | `organization/OrganizationSettingsPage` | 1 |
| `branch/BranchListPage` | 2 | `payment/PaymentSection` | 1 |
| `company/CompanyListPage` | 2 | `pipeline/PipelineSelect` | 1 |
| | | `source/FieldMappingEditor` | 1 |
| | | `source/SourceFormPage` | 1 |
| | | `vehicle/VehicleFormPage` | 1 |

**No se tocaron**, por la misma decisión del §44: `CompanySelect`, `ContactSelect` (en `features/opportunity/`), `VehicleSelect` y `OpportunitySelect` (en `features/activity/`) — son buscadores server-side con otra arquitectura. No quedó ningún `<select multiple>`: el último se había resuelto en el ítem 1.

**Decisiones de implementación:**

- **Animación por keyframes y no por transition** (`@keyframes ds-menu-in`, 140 ms `ease-out`, opacidad 0→1 y `translateY(-4px)`→0). Los dos paneles se montan y desmontan del DOM (`{open ? <div…> : null}`), no alternan una clase abierto/cerrado, y una `transition` sobre un nodo recién montado no dispara: no hay estado anterior desde el cual interpolar. Una keyframe sí corre sola al montarse, así que la entrada se resuelve en CSS puro, sin tocar el estado de `Select.tsx` ni de `MultiSelect.tsx` y sin arriesgar los tests de apertura/cierre.
- **Sin animación de salida**, mismo criterio que el pop up de "Ver detalle" (§34): animar el cierre obliga a demorar el desmontaje en React, que es otro orden de cambio y le agrega un estado intermedio a dos componentes cuyo ciclo abrir/cerrar ya está cubierto por tests.
- **`prefers-reduced-motion`:** la regla vive **dentro** de `@media (prefers-reduced-motion: no-preference)`, y no como un `animation: none` bajo `reduce`. Así el movimiento nunca llega a declararse para quien lo desactivó.
- **`PipelineSelect` y `StageSelect`** se migraron con el patrón exacto de `BranchSelect` (§44): mismas props públicas, `Select` cuando la query resolvió y el rótulo con el aviso de carga/error debajo mientras tanto. Ningún formulario que los consume cambió. La `<option value="" disabled>` de antes pasa a ser `emptyOption` **ofrecida solo mientras no hay valor** (`value ? undefined : {…}`), que es el mismo comportamiento: se lee como placeholder con el campo vacío y no se puede volver a vacío una vez elegido. Es el criterio que ya usaba `UserSelect` con `clearable={false}`. El `StageSelect` sin proceso de venta sigue siendo un control deshabilitado cuyo texto es el cartel "Elegí primero un proceso de venta…".
- **Un solo componente extraído: `design-system/SortOrderSelect.tsx`.** El filtro "Orden" se repetía **letra por letra** en 12 listados: mismo rótulo, mismas dos opciones, mismo tipo. Doce copias del mismo bloque de siete líneas era duplicación real, así que se extrajo. Vive en el design system y no en un feature porque no pertenece a ninguno: cada feature declara su propio `SortOrder = "asc" | "desc"` (doce declaraciones estructuralmente idénticas) y todas encajan en su `SortDirection` por compatibilidad estructural, sin que el componente importe nada de `features/`.
- **Todos los demás se migraron in place**, sin extraer. "Ordenar por" ofrece campos distintos en cada listado, y los filtros de Estado/Tipo/Condición son de enums distintos: un componente compartido habría necesitado parametrizar todo lo que los distingue.
- **Un caso queda inline a propósito:** el "Orden" de `IngestionEventListPage` no usa `SortOrderSelect`. Es el único de los trece cuyos rótulos no son "Descendente"/"Ascendente": ahí lo que se ordena es siempre una fecha y dice "Más recientes primero"/"Más antiguos primero".
- **`Select` gana una prop, `labelHidden`**, que oculta el rótulo con `.ds-sr-only`. Un único consumidor: la columna Rol de `UserListPage`, donde el `<select>` vivía en una celda con `aria-label` y sin rótulo visible. El `<label>` sigue en el DOM y asociado por `for`/`id`, así que el nombre accesible no cambia ("Rol de \<nombre\>") — por eso se oculta en vez de no renderizarlo.
- **Los `<select>` que estaban dentro de un `FormField` pasan a montarse sueltos**, como ya hacían `UserSelect` y compañía: `FormField` **es** un `<label>` y `Select` trae el suyo, así que anidarlos daría HTML inválido y un `getByLabelText` ambiguo. El asterisco de obligatorio, que antes se pasaba como `label={<span className="ds-required">…</span>}`, ahora se pide con la prop `required` de `Select`, que pone las dos cosas juntas (marca + bloqueo).
- **Genérico explícito en dos lugares**, donde la lista de opciones NO cubre todo el tipo: `SourceFormPage` (`Select<SourceType>`, porque la lista no ofrece `EXTERNAL_DB`) y `UserListPage` (`Select<"ADMIN" | "USER">`, porque `Role.name` es `string` en el contrato). En los dos, un valor fuera de la lista deja el control en blanco — exactamente lo que hacía el `<select>` con un `value` sin `<option>`.
- **Callbacks sin cast.** `Select` admite `""` en `onChange` aunque `T` no lo incluya (es lo que produce la fila vacía, §44). En los selectores sin `emptyOption` el `""` es inalcanzable, y en vez de castear se descarta con un guard (`if (value) setSortBy(value)`). Donde el estado ya admite `""` (los filtros: `Status | ""`), el callback se pasa directo.

**Hallazgos al implementar:**

- **`.ds-table-wrap` le recortaba el panel al `Select` de la grilla de Usuarios.** El wrapper lleva `overflow-x: auto` para que una tabla ancha scrollee sola, y por regla de CSS el eje que queda en `visible` computa a `auto` en cuanto el otro no lo está: es un contenedor de scroll en los dos ejes. Un panel abierto en las últimas filas quedaba cortado. Se arregló con `.ds-table-wrap:has(.ds-select-menu) { overflow: visible }` — solo mientras hay un panel abierto adentro. **Tradeoff aceptado y anotado en el CSS:** durante ese rato la tabla no puede scrollear en horizontal, y si estuviera scrolleada volvería al margen izquierdo; es aceptable porque la tabla de Usuarios tiene cinco columnas angostas y no llega a desbordar, y se corrige solo al cerrar. La solución de fondo (renderizar el panel en un portal con posición fija) es otro orden de cambio y no entra en esta fase. Si más adelante una tabla **ancha** necesita un `Select` adentro, el selector se acota con una clase en vez de generalizar esto.
- **El rótulo del `Select` se leía distinto que el de un `FormField` vecino.** `FormField` pinta su rótulo con `.ds-field-label` (13px/600/mutado) y `Select` traía el `label` base (14px/500). Hasta ahora eso lo tapaba `.ds-field-grid label`, que alcanza porque casi todos los formularios usan esa grilla; al migrar el resto aparecieron contenedores que no la usan (`.ds-payment-fields`, `.ds-field-row`, las filas de `QuoteFormPanel` y de `FieldMappingEditor`, el `<form>` suelto de `QrFormDialog`). La regla se ató al componente (`.ds-select > label`) en vez de al contenedor de turno. Dentro de la píldora de filtros no aplica: el selector de `.ds-filters`/`.ds-board-toolbar` es más específico y ahí el rótulo sigue heredando el tamaño de la píldora. Efecto lateral bueno: el `BranchSelect` suelto del diálogo de QR, que desde el §44 se veía más grande que sus vecinos, quedó alineado.
- **La píldora de filtros generaliza bien.** El chevron, el alto y la alineación del §44 funcionan igual en Condición, Etapa, Estado, Tipo, Fuente y Orden — no hizo falta ninguna regla nueva. Lo único que se nota es que el ancho fijo de 10rem que puso el §44 deja holgura con valores cortos ("Todas") al lado del trigger de `MultiSelect`, que se mide por contenido. **Se dejó como está a propósito:** un input que se mide por contenido tiembla mientras se tipea en el buscador, que es peor que la holgura. Es la misma apariencia que ya tenía Sucursal desde el §44, no una regresión nueva.
- **El orden de las opciones de "Orden" en Usuarios estaba invertido.** Era el único de los doce que listaba Ascendente primero. Al salir del componente compartido quedó como los otros once (Descendente primero). El valor por defecto no cambió.
- **Migrar los tests fue la mitad del trabajo:** 108 llamadas a `user.selectOptions` en 24 archivos pasaron a `chooseSelectOption`. Tres patrones más tuvieron que cambiar, y valen como referencia para cualquier `<select>` que se migre después:
  - **`toHaveValue` ve el rótulo, no el valor.** `toHaveValue("MEETING")` pasa a `toHaveValue("Reunión")`, `toHaveValue("src2")` a `toHaveValue("Feria")`. El valor interno se sigue afirmando donde importa: en el cuerpo del POST/PATCH o en la query string.
  - **Listar las opciones es abrir el panel.** `within(select).getAllByRole("option")` y `Array.from(select.options)` no existen más: se usa `listSelectOptions`. Con eso, el **valor** de cada fila deja de ser observable en el DOM; donde un test afirmaba el par `[value, texto]` ahora afirma el texto, y el valor queda cubierto por el assert de la request.
  - **`listSelectOptions` deja el panel abierto, y el listbox comparte el rótulo con el input** (`aria-labelledby`). Después de llamarlo, `getByLabelText("Etapa")` encuentra dos elementos: hay que pedir `getByRole("combobox", { name: … })`.
- **Esperar a que cargó la lista ya no se puede hacer mirando una opción.** Los `waitFor(() => getByText("Ventas"))` de los formularios pasan a `findByRole("combobox", { name: "Proceso de venta" })` (la lista cargada) o a `expect(getByLabelText("Etapa")).toBeEnabled()` (StageSelect, que sin proceso de venta renderiza el cartel deshabilitado).
- **Verificación visual con `/browse`, en claro y oscuro** (`data-theme` del §31), sobre una página temporal de Vite que monta los componentes reales — el magic link del admin en el Supabase local lo bloquea el clasificador, así que se usó el mismo recurso que el §38. Revisado: el "Ordenar por"/"Orden" de Empresas (el caso que disparó el ítem), una píldora con Condición y Etapa al lado de un `MultiSelect` (para confirmar que el CSS de la píldora generaliza en un caso que no es Sucursal), un formulario con Tipo/Moneda/Financiación intercalados con campos de `FormField`, y el `Select` de la celda de la tabla. En los cuatro: chevron correcto, panel por encima de la tarjeta siguiente sin recortes, fila elegida con el acento y el check, rótulos parejos, y ningún ícono decorativo. La animación se verificó por CSSOM (`animation-name: ds-menu-in`, `140ms`, `ease-out`, declarada solo bajo `no-preference`): `Emulation.setEmulatedMedia` no está en la allowlist de CDP de `/browse`, así que el caso `reduce` se comprobó estructuralmente y no por captura. **Se ve** como un desplegable que aparece fundiéndose mientras baja cuatro píxeles hasta su lugar: se nota que entró, no se nota como una animación.

- **Rebase sobre el §47 y el §48.** Los dos aterrizaron en `master` mientras esta rama estaba abierta y tocan los mismos archivos: el §47 renombró "Pipeline" a "Proceso de venta" (rótulos de `PipelineSelect`/`StageSelect` y sus tests) y el §48 hizo que "Motivo de pérdida" aparezca solo con Estado = Perdida, justo al lado del `Select` de Estado que migra esta ronda. Se resolvió quedándose con el control nuevo y con el texto y la condición de ellos: el cartel del `StageSelect` sin proceso de venta, los rótulos de los tests y el gate de "Motivo de pérdida" son los de `master`. El número de sección quedó en §46 igual, entre el §45 y el §47.

**Tests:**

- **Ninguno nuevo y ninguno borrado:** la migración no agrega comportamiento. **24 archivos de test actualizados**, más `PipelineSelect.test.tsx` y `StageSelect.test.tsx`, que se reescribieron al molde de `BranchSelect.test.tsx` del §44 (incluida la verificación de que tipear filtra local y no dispara otra request).
- Dos asserts que ya no tienen sentido se reemplazaron en vez de borrarse: el `queryByRole("textbox")` de `PipelineSelect` ("no hay buscador") pasa a "tipear no dispara otra request", que es lo que de verdad se quería garantizar; y los pares `[value, texto]` de las opciones de Etapa/Estado pasan a la lista de rótulos, con el valor cubierto por el assert de la request.
- **Suite de frontend en verde: 138 archivos y 1340 tests** — el mismo conteo que el §44 y el §45, como corresponde a una migración sin comportamiento nuevo. `tsc -b`, ESLint y Prettier limpios. Sin backend, sin migraciones y sin dependencias nuevas.
...
**Backend: 904 unitarios, todos en verde** (antes del ítem, con el 70 ya mergeado en `master`: 899). Neto +5, porque además de los que se agregaron se fueron dos que probaban código borrado.
...
**Frontend: 156 archivos, 1677 casos, todos en verde** (antes del ítem, con el 70 ya mergeado: 156 archivos, 1674). Ningún archivo nuevo.
...
**Backend: 923 unitarios en verde** (antes del ítem: 915) **y 910 de integración** (antes: 901).
...
**La suite de integración en esta máquina no dio una corrida limpia, y el motivo no es este ítem.** Primera corrida: fallaron los 7 casos del worker de canales de Google Calendar porque el `.env` de este worktree no define `GOOGLE_WEBHOOK_URL` (el CI la define en `ci.yml`). Con esa variable seteada igual que el CI, dos corridas más: **909/910** y **907/910**, con fallas **distintas en cada corrida** y en archivos que no tocan sucursales ni tools (`ingest.controller` en el `after` de limpieza, `opportunityRevenueSeries`, el drenado de `promotion.service`); `ingest.controller.integration-test.ts` solo pasa **30/30 dos veces seguidas**. El contenedor `supabase_db_Plataforma_CRM` es **uno solo para todos los worktrees** y hay otras dos ramas armándose en paralelo, así que lo más probable es que sean suites de otros worktrees corriendo contra la misma base al mismo tiempo. **La referencia es el job `integration` del CI**, que levanta su propio Postgres desde cero. Los 9 casos nuevos pasaron en las tres corridas.
