...
- **`agent.controller.integration-test.ts` (25 casos, 5 nuevos):** el endpoint de traducción por HTTP real (ADMIN traduce y recibe `guardrails` + `descartado`, sin tools y sin `model`; texto vacío que no llama al proveedor; USER 403, sin token 401, texto de 4001 caracteres 400; respuesta ininteligible 502), y que `guardrails`/`guardrailsText` **van juntos o no van** en el PATCH (cada uno solo es 400 y la fila no cambia; un PATCH que no menciona ninguno de los dos sigue siendo válido). Los casos que ya existían se ajustaron al contrato nuevo: `guardrailsText` en el cuerpo mínimo, el texto que se persiste tal cual, y el `""` del agente recién creado. Corridos contra el Supabase local: **25 en verde** (no van a CI, que sigue corriendo solo los unitarios).
