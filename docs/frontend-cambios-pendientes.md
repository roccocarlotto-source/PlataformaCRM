...
**Backend: 879 unitarios y 848 de integración, todos en verde.** (En la primera corrida fallaban 7 de integración, los de `googleCalendarSync.integration-test.ts`: el `.env` local no define `GOOGLE_WEBHOOK_URL`, que el CI sí trae — exportándola a mano da 848/848, que es la corrida que se reporta acá.) Las tres suites de automatizaciones se corrieron además por separado: **40 casos, todos en verde**.
