...
- ⏳ **M6 (módulo Activity) implementado y verificado — cierre del ciclo
  ...
  **Routing**: en M6, `/activities` (lectura) quedó fuera del `AdminRoute`
  porque `GET /api/activities` era abierto a cualquier rol autenticado.
  **Desde el ítem 25 de `docs/frontend-cambios-pendientes.md` ya no**: la
  ruta del backend sigue sin `authorize` (la usa "Mis tareas"), pero
  `listActivities`/`getActivityById` reciben el actor y acotan a un USER a
  lo asignado a sí mismo (una ajena por id es 404), así que el listado
  completo `/activities` pasó a vivir dentro del mismo `AdminRoute` que
  `/activities/new` y `/activities/:id/edit`; `/tasks` sigue fuera.
