...
export async function lockBranchForUpdate(
  ...
  const filas = await db.$queryRaw<
    { id: string }[]
  >`SELECT id FROM branches WHERE id = ${id}::uuid AND organization_id = ${organizationId}::uuid FOR UPDATE`;
  if (filas.length === 0) {
    throw new Error(
      `lockBranchForUpdate: no existe la branch ${id} en la organización ${organizationId} — no se tomó ningún lock`,
    );
  }
}
